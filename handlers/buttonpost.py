...
# 本地短期会话状态（fallback / 兼容）
state = {}

# 兼容 pending_redeem 模块（若存在）
try:
...
async def handle_cover_message_if_expected(client, message):
    """
    处理封面：不��强制要求 reply_to。
    增加保护：若用户当前处于 explicit_upload 流，则不抢占（优先让 upload 处理）。
    如果 flowguards 表示当前为 buttonpost 流程（或本地 state 处于 waiting_photo），则接管该图片作为封面。
    否则记录冲突并按节流提示用户（避免无响应）。
    """
    try:
        user_id_tmp = getattr(message.from_user, "id", None)
        if user_id_tmp and user_id_tmp in pending_redeem:
            print(f"[buttonpost] skipping cover handling for user {user_id_tmp} because pending_redeem exists")
            return False
    except Exception:
        pass
    ...
    # EARLY RETURN: 如果用户当前在 explicit_upload 流中，优先让 upload ���理，不抢占
    if flowguards:
        try:
            g_check = flowguards.get_flow(user_id)
            if g_check and isinstance(g_check, dict) and g_check.get("flow") == "explicit_upload":
                print(f"[buttonpost.debug] user {user_id} in explicit_upload, skipping buttonpost.handle_cover")
                return False
        except Exception:
            pass
    ...
    try:
        ...
        if not photo_id:
            print("[buttonpost] cannot extract photo file_id")
            return False
...
async def process_buttonpost_text(client, message):
    ...
    # EARLY RETURN: 如果用户当前在 explicit_upload 流中，放弃文本处理，让 upload 接管
    if flowguards:
        try:
            g_check = flowguards.get_flow(user_id)
            if g_check and isinstance(g_check, dict) and g_check.get("flow") == "explicit_upload":
                print(f"[buttonpost.debug] user {user_id} in explicit_upload, skipping buttonpost.process_text")
                return False
        except Exception:
            pass
...
def register_buttonpost(app):
    ...
    @app.on_callback_query(filters.regex("^back_to_edit$"))
    async def back_to_edit(client, cb):
        user_id = cb.from_user.id
        if user_id in state:
            state[user_id]["step"] = "editing"
            try:
                ...
                pass
            await cb.message.edit_text("✅ 已返回编辑模式", reply_markup=get_edit_keyboard(user_id))
        await cb.answer()

    @app.on_callback_query(filters.regex("^add_description$"))
    async def add_description_cb(client, cb):
        user_id = cb.from_user.id
        if user_id not in state:
            await cb.answer("请先开始生成广告图", show_alert=True)
            return
        state[user_id]["step"] = "waiting_description"
        try:
            if flowguards:
                flowguards.update_step(user_id, {"expect": "waiting_description"})
        except Exception:
            pass
        await cb.message.edit_text("✏️ 请输入要显示在广告图下方的描述文本（支持简短 HTML），发送内容后会保存为广告描述：")
        await cb.answer()
    ...
    async def inline_query(client, iq):
        ...
        if not post_file_id:
            ...
            if not post_file_id:
                try:
                    cursor.execute("SELECT telegram_file_id FROM files WHERE batch_id=? AND file_type='cover' ORDER BY rowid DESC LIMIT 1", (batch_id,))
                    frow = cursor.fetchone()
                    if frow:
                        source_file_id = frow[0] if isinstance(frow, (list, tuple)) else frow["telegram_file_id"]
                        if source_file_id:
                            try:
                                print(f"[buttonpost.inline] 回填：将文件 {source_file_id} 重新发送到 POST_CHANNEL")
                                sent = await client.send_photo(POST_CHANNEL, source_file_id, caption=post_caption or None)
                                new_file_id = None
                                new_message_id = None
                                try:
                                    if hasattr(sent, "photo") and sent.photo:
                                    ...
                                    pass
                                if new_file_id:
                                    post_file_id = new_file_id
                                    try:
                                        with db_lock:
                                            cursor.execute("UPDATE batches SET post_file_id=?, post_message_id=? WHERE batch_id=?", (post_file_id, new_message_id, batch_id))
                                            conn.commit()
                                    except Exception as e:
                                        print(f"[buttonpost.inline] 写回 post_file_id 失败: {e}")
                            except Exception as e:
                                print(f"[buttonpost.inline] resend to POST_CHANNEL 失败: {e}")
                except Exception as e:
                    print(f"[buttonpost.inline] 从 files 表回填失败: {e}")

        if not post_file_id:
            print(f"[buttonpost.inline] 最终无法获取 post_file_id for post {num}")
            return
