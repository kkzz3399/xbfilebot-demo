...
def main_menu(user_id):
    ...
    try:
        if is_admin(user_id):
            ...
            flat.append(InlineKeyboardButton("📊 上传统计", callback_data="stats"))
...
# 以下函数保留原有行为（用于上传流程和文件夹管理）
# 不对齐主菜单的“每行两列”约束，以保证细节页面展示完整

def upload_menu(batch_id, forward_allowed, has_files=False):
    """
    上传流程的交互菜单（用于已创建上传批次时）
    """
    buttons = [
...
def folder_list_menu(user_id, folders, from_finish=False):
    """
    列出文件夹（管理我的文件夹）时的菜单，保持原有每行多项布局，便于展示操作按钮
    参数 folders 期望为 [(batch_id, folder_name, photos, videos, other, forward_allowed), ...]
    """
    buttons = []
...
def folder_detail_menu(batch_id, folder_name, p, v, o):
    """
    单个文件夹详情页面的返回 (text, InlineKeyboardMarkup)
    """
...
def uploading_menu(batch_id, forward_allowed):
    """
    文件夹名设置完成后在上传过程中显示的菜单（与 upload_menu 相似）
    """
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(
            "😊 可以转发" if forward_allowed else "😢 禁止转发",
