...
# 完整版 VIP 回调处理模块（整合：下单、二维码、轮询、手动确认、管理员面板、分步 CDK 生成与自定义数量）
# 设计目标：
# - 稳健读取/回显订单字段（兼容 sqlite3.Row 或 tuple）
# - 避免 DB 锁死（不在持锁时再次取锁）
# - 管理员分步生成 CDK（选择套餐 -> 选择数量 -> 生成；支持自定义数量通过回复）
# - 在关键路径添加可观察的 print 日志，便于定位问题
# 依赖：db.cursor, db_lock, vip_store, vip_keyboard, config
...
HAS_SEGNO = True
try:
    importlib.util.find_spec("segno")
except Exception:
    HAS_SEGNO = False
...
async def _process_mark_paid(client, order_id, admin_chat_id=None, admin_msg_id=None, admin_uid=None):
    try:
        ...
        # ensure we have order_user_id and order_days
        if not order_user_id:
            new_order = _fetch_order(order_id)
            if new_order:
                order_user_id = new_order["user_id"]
                order_days = new_order["duration_days"]
        ...
        try:
            if order_user_id:
                ...
                print(f"[vipscenter] sent code to user {order_user_id} for order {order_id}")
        except Exception:
            print("[vipscenter] failed to send message to user", traceback.format_exc())

        # send full order summary to user (with correct fields)
        try:
...
def register_vip_callbacks(app):
    ...
    async def _vip_buy_pkg(client, cb):
        ...
        if not pay_addr:
            await cb.message.edit_text("支付地址未配置，请联系管理员处理。")
            await cb.answer()
            return
    ...
    async def _order_paid_notify(client, cb):
        ...
        if not HAS_AIOHTTP:
            try:
                await cb.answer("管理员已被通知。自动轮询不可用，需管理员手动确认到账。", show_alert=True)
            except Exception:
                pass
            try:
                await cb.message.edit_text("已通知管理员。自动检测不可用，请等待管理员手动确认。")
            except Exception:
                pass
            return

        try:
            await cb.answer("管理员已被通知，正在轮询链上确认，请耐心等待…", show_alert=False)
        except Exception:
            pass
        try:
            await cb.message.edit_text("已通知管理员，系统将自动轮询确认到账。如确认到账后将自动发放卡密或管理员可手动确认。")
        except Exception:
            pass
    ...
    # ADMIN: list VIP users with buttons
    ...
    # ADMIN: remove vip from list (quick delete)
    ...
    # Message listener for admin custom CDK quantity (registered here to keep scope)
    ...
    async def _noop_vips_gen_cdk_dup(client, cb):
        # no-op duplicate guard - can be removed if no duplicate handler exists
        try:
            await cb.answer()
        except Exception:
            pass
